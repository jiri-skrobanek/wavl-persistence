\begin{figure}
\begin{center}
\begin{tikzpicture}[sibling distance=32pt]
\Tree
[ \edge node[auto=right]{1 or 2}; [.y
    \edge node[auto=right] {0};
    \node[draw,triangle]{~};
    \edge node[auto=left] {1};
    \node[draw,triangle]{~};
] ]
\end{tikzpicture}
\qquad
\begin{tikzpicture}[sibling distance=32pt]
\Tree
[ \edge node[auto=right]{0 or 1}; [.y
    \edge node[auto=right] {1};
    \node[draw,triangle]{~};
    \edge node[auto=left] {2};
    \node[draw,triangle]{~};
] ]
\end{tikzpicture}
\end{center}
{\small Situation on the left displays is prior to the step and situation on the right after the step. Edges are annotated by rank differences between the endpoint. This convention is followed for depictions of all steps in this section.}
\caption{Promote step}
\end{figure}

\begin{figure}
\begin{center}
\begin{tikzpicture}[sibling distance=32pt]
\Tree
[ \edge node[auto=right]{1 or 2}; [.y
    \edge node[auto=right] {0};
    [   .x 
        \edge node[auto=right] {1}; 
        \node[draw,triangle]{A}; 
        \edge node[auto=left] {2};
        \node[draw,triangle]{B}; ]
    \edge node[auto=left] {2};
    \node[draw,triangle]{C};
] ]
\end{tikzpicture}
\qquad
\begin{tikzpicture}[sibling distance=32pt]
\Tree
[ \edge node[auto=right]{1 or 2}; [.x
    \edge node[auto=right] {1};
    \node[draw,triangle]{A};
    \edge node[auto=left] {1};
    [   .y 
        \edge node[auto=right] {1}; 
        \node[draw,triangle]{B}; 
        \edge node[auto=left] {1}; 
        \node[draw,triangle]{C}; ]
] ]
\end{tikzpicture}
\end{center}
\caption{Insert rotation step}
\end{figure}

\begin{figure}
\begin{center}
\begin{tikzpicture}[sibling distance=8pt]
\Tree
[ 
    \edge node[auto=right]{1 or 2}; 
    [
        .y
        \edge node[auto=right] {0};
        [   
            .x 
            \edge node[auto=right] {2}; 
            \node[draw,triangle]{A}; 
            \edge node[auto=left] {1};
            [
                .w
                \node[draw,triangle]{B};
                \node[draw,triangle]{C}; 
            ]
        ]
        \edge node[auto=left] {2};
        \node[draw,triangle]{D};
    ] 
]
\end{tikzpicture}
\qquad
\begin{tikzpicture}[sibling distance=8pt]
\Tree
[ \edge node[auto=right]{1 or 2}; [.w
    \edge node[auto=right] {1};
    [ 
        .x
        \edge node[auto=right] {1};
        \node[draw,triangle]{A};
        \node[draw,triangle]{B};
    ]
    \edge node[auto=left] {1};
    [   .y 
        \node[draw,triangle]{C}; 
        \edge node[auto=left] {1}; 
        \node[draw,triangle]{D}; ]
] ]
\end{tikzpicture}
\end{center}
\caption{Insert double rotation step}
\end{figure}