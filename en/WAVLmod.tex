...
Vertex $x$ has just been inserted into the upper tree. Promotion path (thick) has created during rebalancing in the lower tree. We see that this pattern can be extended to make promotion path arbitrarily long.
\end{figure}
...
Layout of a vertex can be modified slightly. In a vertex that is the top of a modifying path, it is possible to store key of the vertex where modifying path ends instead of pointer to it.
